        ...
        if ($v != '') {
            if ($v == 'delete_img') {
                ...
                if (isset($_REQUEST['img_date' . $k]) && !null == $_REQUEST['img_date' . $k]) {
                    ...
                    $arrTaggedDate[$k] = NULL; //"0000-00-00T00:00:00Z"
...
//        }
        ...
        foreach ($arrValue as $k => $val) {
            ...
            $txtlocationplan = "kuldeep";
            
            $extra_path = $BuilderName . "/" . strtolower($ProjectName) . "/";
            
            if (!$_FILES["img"]["tmp_name"][$k]) {
                $ErrorMsg["ImgError"] .= "Problem in Image Upload Please Try Again.";
                break;
            } else {

                if ($_REQUEST['PType'][$k] == "Location Plan")
                    $image_type = "locationPlan";
                elseif ($_REQUEST['PType'][$k] == "Layout Plan")
                    $image_type = "layoutPlan";
                elseif ($_REQUEST['PType'][$k] == "Site Plan")
                    $image_type = "sitePlan";
                elseif ($_REQUEST['PType'][$k] == "Master Plan")
                    $image_type = "masterPlan";
                elseif ($_REQUEST['PType'][$k] == "Cluster Plan")
                    $image_type = "clusterPlan";
                elseif ($_REQUEST['PType'][$k] == "Construction Status")
                    $image_type = "constructionStatus";
                elseif ($_REQUEST['PType'][$k] == "Payment Plan")
                    $image_type = "paymentPlan";
                elseif ($_REQUEST['PType'][$k] == "Elevation")
                    $image_type = "main";
                elseif ($_REQUEST['PType'][$k] == "Amenities")
                    $image_type = "amenities";
                elseif ($_REQUEST['PType'][$k] == "Main Other")
