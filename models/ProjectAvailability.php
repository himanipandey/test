...
class ProjectAvailability extends Model {
    ...
    public static function getInventoryForIndexing($aProjectId){
        ...
        $sql = "select concat_ws('/', rp.PROJECT_Id, rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS, pa.effective_month) as unique_key, concat_ws('/', rp.PROJECT_Id, rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS) as key_without_month, rp.PROJECT_ID, rpp.PHASE_ID, rpo.OPTION_TYPE as UNIT_TYPE, rpo.BEDROOMS, pa.effective_month, rp.PROJECT_NAME, rpp.PHASE_NAME, date_format(rpp.COMPLETION_DATE, '%Y-%m-01') COMPLETION_DATE, date_format(if(rpp.LAUNCH_DATE = 0, rp.PRE_LAUNCH_DATE, rpp.LAUNCH_DATE), '%Y-%m-01') LAUNCH_DATE, rpp.PHASE_TYPE, if(if(rpp.LAUNCH_DATE = 0, rp.PRE_LAUNCH_DATE, rpp.LAUNCH_DATE) = pa.effective_month, ps.supply, null) supply, if(if(rpp.LAUNCH_DATE = 0, rp.PRE_LAUNCH_DATE, rpp.LAUNCH_DATE) = pa.effective_month, ps.launched, null) launched, pa.availability as inventory from resi_project rp inner join resi_project_phase rpp on rp.PROJECT_ID = rpp.PROJECT_ID and rpp.version = 'Website' and rpp.status = 'Active' inner join listings li on rpp.PHASE_ID = li.phase_id and li.status = 'Active' inner join resi_project_options rpo on rpo.OPTIONS_ID = li.option_id and rpo.OPTION_CATEGORY = 'Logical' inner join project_supplies ps on li.id = ps.listing_id and ps.version = 'Website' inner join project_availabilities pa on ps.id = pa.project_supply_id $condition group by pa.id order by rp.PROJECT_Id, rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS, pa.effective_month";
        return self::find_by_sql($sql);
    }
}
