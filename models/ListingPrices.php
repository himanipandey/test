...
class ListingPrices extends ActiveRecord\Model
{
    ...
    public static function getPriceForIndexing($aPhaseId){
        ...
        $sql = "select concat_ws('/', rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS, lp.effective_date) as unique_key, concat_ws('/', rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS) as key_without_month, rpp.project_id, rpp.phase_type, FLOOR(avg(if(rpo.SIZE is null or rpo.SIZE = 0, if(rpo.CARPET_AREA = 0, null, rpo.CARPET_AREA), rpo.SIZE))*avg(lp.price_per_unit_area)) as AVERAGE_TOTAL_PRICE, lp.effective_date as effective_month, rpp.PHASE_NAME, FLOOR(avg(if((rpo.SIZE = 0 or rpo.SIZE is null) and rpo.CARPET_AREA is not null and rpo.CARPET_AREA != 0, lp.price_per_unit_area/" . ResiProjectOptions::$carpet_area_factor . ", lp.price_per_unit_area))) as average_price_per_unit_area from resi_project_phase rpp inner join listings li on (rpp.PHASE_ID = li.phase_id and li.listing_category='Primary') inner join resi_project_options rpo on li.option_id = rpo.OPTIONS_ID and rpo.OPTION_CATEGORY = 'Actual' inner join listing_prices lp on li.id = lp.listing_id $condition group by unique_key order by rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS, lp.effective_date";
