...
class ListingPrices extends ActiveRecord\Model
{
    ...
    public static function getPriceForIndexing($aProjectId){
        ...
        $sql = "select concat_ws('/', rp.PROJECT_Id, rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS, lp.effective_date) as unique_key, concat_ws('/', rp.PROJECT_Id, rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS) as key_without_month, rp.PROJECT_ID, rpp.PHASE_ID, rpo.OPTION_TYPE as UNIT_TYPE, rpo.BEDROOMS, FLOOR(avg(rpo.SIZE)) as AVERAGE_SIZE, GROUP_CONCAT(FLOOR(rpo.SIZE)) as SIZE, FLOOR(avg(rpo.SIZE)*avg(lp.price_per_unit_area)) as AVERAGE_TOTAL_PRICE, lp.effective_date as effective_month, concat(year(effective_date), '-', LPAD(((month(effective_date)-1) DIV 6)*6+1, 2, 0), '-01') half_year, concat(year(lp.effective_date), '-01-01') year, concat(year(effective_date), '-', LPAD(((month(effective_date)-1) DIV 3)*3+1, 2, 0), '-01') quarter, rp.PROJECT_NAME, rpp.PHASE_NAME, rpp.COMPLETION_DATE, if(rpp.LAUNCH_DATE = 0, rp.PRE_LAUNCH_DATE, rpp.LAUNCH_DATE) as LAUNCH_DATE, rb.BUILDER_ID, rb.BUILDER_NAME, c.CITY_ID, c.LABEL as CITY_NAME, s.SUBURB_ID, s.LABEL as SUBURB_NAME, l.LOCALITY_ID, l.LABEL as LOCALITY_NAME, rpp.PHASE_TYPE, FLOOR(avg(lp.price_per_unit_area)) as average_price_per_unit_area from resi_project rp inner join resi_builder rb on rp.BUILDER_ID = rb.BUILDER_ID inner join locality l on rp.LOCALITY_ID = l.LOCALITY_ID inner join suburb s on l.SUBURB_ID = s.SUBURB_ID inner join city c on s.CITY_ID = c.CITY_ID inner join resi_project_phase rpp on rp.PROJECT_ID = rpp.PROJECT_ID and rpp.version = 'Website' and rpp.status = 'Active' inner join resi_project_options rpo on rp.PROJECT_ID = rpo.PROJECT_ID and rpo.OPTION_CATEGORY = 'Actual'  inner join listings li on rpo.OPTIONS_ID = li.option_id and li.status = 'Active' inner join listing_prices lp on li.id = lp.listing_id and lp.version = 'Website' and lp.status = 'Active' $condition group by rp.PROJECT_Id, rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS, lp.effective_date order by rp.PROJECT_Id, rpp.PHASE_ID, rpo.OPTION_TYPE, rpo.BEDROOMS, lp.effective_date";
