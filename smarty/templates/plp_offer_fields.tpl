			{for $k=1 to 24}
							<table style="border:1px solid#ccc;padding:5px;{if $k<=$noi}display:block{else}display:none{/if}" id="inst-{$k}">
								...
								<tr>
								  <td>
								    ...
								    <div id="plpMnth-wrapper-{$k}" style="display:{if ($k==1) || ($k==$noi)}none{/if}">
								       	<input type="radio" name="plp_period_{$k}" value="months" onclick="$('#btnSave').attr('disabled',true)" {if is_numeric($plp_arr['offer_period'][$k])}checked{/if}/>
										 <input type="text" size="2"  maxlength="2" id="plp_Months_{$k}" name="plp_Months_{$k}" onkeyup="$('#btnSave').attr('disabled',true)" style="width:50px" onkeypress='return isNumberKey(event)' value="{if is_numeric($plp_arr['offer_period'][$k])}{$plp_arr['offer_period'][$k]}{/if}"/>
										 Months 
									</div>
								...
								<tr>
								<td>
								    <b><font color = "red">*</font>Instalment Price : </b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="plp_price_{$k}" value="percent" onclick="$('#btnSave').attr('disabled',true)" {if $plp_arr['offer_price_type'][$k]=='Percent'}checked{/if}/> 
								     <input type="text" size="3"  maxlength="3"  name="plp_Per_{$k}" id="plp_Per_{$k}" onkeyup="$('#btnSave').attr('disabled',true)" style="width:50px" onkeypress='return isNumberKey(event)' value="{if $offer_price_type=='Percent'}{$plp_arr['offer_price'][$k]}{/if}"/>
								     Percent 
								  </td>
								  <td>
