      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0><TBODY>
        ...
        <tr>
          ...
          <td width="100" align="left" >
            <div id='suburbs' data-role="tagsinput"></div>
            <div><label id="removetext2" style="color:green; font-weight: bold;"></label><a href="#" onclick="showHier();"><b>See Hierarchy</b></a></div>
          </td>
          ...
          <td  height="25" align="right" style="padding-left:5px;">
              <b>Add Suburbs:</b>
                           ...
                           <td height="50%" align="left">
                            ...
                            <select name="parentId" id = "parentSelect" class="suburbId" STYLE="width: auto" >
                            
                            {foreach from=$suburbSelect key=k item=v}
                                <option value="{$v.id}" {if $v.id==$parent_sub_id} selected = "selected" {/if}>{$v.label}</option>
                            {/foreach}
                            </select> 
                            <button type="button" id="button_suburb" align="left"><b>Save Suburb</b></button><label align="left" id="onclicktext2" style="color:green; font-weight: bold;"></label>
                            </div>
                            </td>
...
if(!jQuery.isEmptyObject({$landmarkJson}) && {$landmarkJson}!==null && {$landmarkJson}!==undefined){
...
if(!jQuery.isEmptyObject({$attachedSuburbJson})){
  var a= {$attachedSuburbJson};
  var suburbAliases = [];
  for(var i in a){
      suburbAliases.push([i, a [i]]);
  }
  for (index = 0; index < suburbAliases.length; ++index) {
    var elm = suburbAliases[index];
      $('#suburbs').tagsinput('add', { "value": elm[1].id , "text": elm[1].label, "type": "suburb"    }); 
  }
}
...
$("#suburbs").on('itemRemoved', function(e) {
   
    var suburbId = e.item.value;
    ...
    $.ajax({
            type: "POST",
            url: '/saveAliases.php',
            data: { suburbId : suburbId, localityId : localityId, task : 'dettachSuburb' },
            success:function(msg){
              //alert(msg);
               if(msg == 1){
                //alert(msg);
                $("#removetext2").text("Suburb Successfully Removed.");
                 
               }
               if(msg == 2){
                $("#removetext2").text("Suburb Already Removed.");
                   
                   //location.reload(true); 
               }
               if(msg == 3){
                $("#removetext2").text("Error in Removing Suburb.");
                   
               }
               if(msg == 4){
                $("#removetext2").text("No Suburb Selected.");
...
var options, d, selectedItem;
 ...
 $.widget( "custom.catcomplete", $.ui.autocomplete, {
   ...
   $(function() {
    ...
    $("#button_suburb").click(function(){
     
          
          
          var suburbId = $("#parentSelect").val();
          var label = $("#parentSelect option:selected").text();
          var localityId = {$localityid};
          //var aliasTableName = selectedItem.table;
          ...
          $.ajax({
            type: "POST",
            url: '/saveAliases.php',
            data: { suburbId : suburbId, localityId : localityId, task : 'attachSuburb' },
            success:function(msg){
              //alert(msg);
               if(msg == 1){
                //alert("saved");
                $("#onclicktext2").text("Suburb Successfully Added.");
                $('#suburbs').tagsinput('add', { "value": suburbId , "text": label , "type": "suburb"    });
                   //location.reload(true); 
               }
               if(msg == 2){
                $("#onclicktext2").text("Suburb Already Added.");
                   
                   //location.reload(true); 
               }
               if(msg == 3){
                $("#onclicktext2").text("Error in Adding Suburb.");
