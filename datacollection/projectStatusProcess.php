...
if (isset($_SESSION['project-status']['city']) && !empty($_SESSION['project-status']['city'])) {
    if (isset($_SESSION['project-status']['locality']) && !empty($_SESSION['project-status']['locality'])) {
        ...
        $projectsfromDB = getProjectListForManagers($_SESSION['project-status']['city'], $getFlag, $_SESSION['project-status']['suburb']);
    }
...
function prepareDisplayData($data) {
    ...
    if (!empty($pids)) {
        ...
        while ($row = mysql_fetch_assoc($mysql_res)) {
            $prv_asg_dept = explode(",", $row['department']);
            $prevs_depts[$row['PROJECT_ID']] = $row['department'];
            $depts[$row['PROJECT_ID']] = $prv_asg_dept[0];
        }
    }
    foreach ($data as $value) {
        ...
        if (($value['PREV_PROJECT_PHASE'] == 'Audit1' || $value['PREV_PROJECT_PHASE'] == 'Audit2') && strstr($prevs_depts[$value['PROJECT_ID']], $assigned_to_dep[count($assigned_to_dep) - 1])) {
            ...
            if (isset($depts[$value['PROJECT_ID']]) && $depts[$value['PROJECT_ID']] === 'SURVEY')
