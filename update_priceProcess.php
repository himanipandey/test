		...
		if($projectid_type == '') 
		{
                               ...
                               if(
                                    ...
                                    )
                                {
                                 ...
                                 if ($priceChanging && !isUserPermitted('price', 'overrideValidation')) {
                                    ...
                                    if ($price_per_unit_area > $pricePerUnitAreaUpperLimit ||
                                        $price_per_unit_area_dp > $pricePerUnitAreaUpperLimit ||
                                        $price_per_unit_area_fp > $pricePerUnitAreaUpperLimit ||
                                        $price_per_unit_high > $pricePerUnitAreaUpperLimit ||
                                        $price_per_unit_low  > $pricePerUnitAreaUpperLimit)
                                    {
                                ...
                                if($blank_chk == 1)
                                {
                                    if(!is_array($ErrorMsg))
                                    {
                                        ...
                                        $qry = "UPDATE ".RESI_PROJECT_OPTIONS." 
                                                ...
                                                AND
                                                    PROJECT_ID = '".$projectId."'";
                                        $res = mysql_query($qry) or die(mysql_error()." error in updation");
                                        if($res)
                                        {
			...
			if($blank_chk == 1 )
			{
                            if(!is_array($ErrorMsg))
                            {
                                ...
                                $res		=	mysql_query($qry_new) or die(mysql_error()." error in insertion"); 
