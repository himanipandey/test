...
def prod(tag):
  local("git archive --format tar.gz --output /tmp/cms.tar.gz %s" %(tag))
...
#fab -Hsysadmin@108.61.90.200 staging:tagname  this is the command for the code to be deployed on staging server
def staging(tag):
  local("git archive --format tar.gz --output /tmp/cms.tar.gz %s" %(tag))
  put('/tmp/cms.tar.gz', '/tmp')
