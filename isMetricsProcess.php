<?php
    ...
    $qryAllProj = "select count(*) as NEW_PROJECT_COUNT from resi_project as rp 
                    LEFT JOIN _t_resi_project a ON(rp.project_id = a.project_id)
                    where a._t_operation = 'I' AND a._t_transaction_date >= '$newDate' and rp.version = 'Cms'";
    $newAddedProject = ResiProject::find_by_sql($qryAllProj);
   /******code for get all project which have master plans***/
    ...
    $percentProjectWithMasterPlan = ($masterPlan->master_plan_count/$activeProject->project_count)*100;
    ...
    $percentProjectWithLocationPlan = ($locationPlan->location_plan_count/$activeProject->project_count)*100;
    ...
    $activeProjectWithUnderConst = ResiProject::virtual_find('all',array('conditions'=>array("(status = 'Active' OR status = 'ActiveInCms') AND residential_flag = 'Residential' AND locality_id in ($LocIdList) AND  project_status_id = ".UNDER_CONSTRUCTION_ID_1 ),
        "select" => "count(*) as PROJECT_COUNT"));
    $percntProjWithConstImg = ($projectCountOfConstImg->project_count_of_construction_images/$activeProjectWithUnderConst->project_count)*100;
   ...
   $projectWithLaunchGrtrOct2012 = ResiProject::virtual_find('all',array('joins' => $joinOptions, 
        'conditions'=>array("($active) 
            AND resi_project.residential_flag = 'Residential' AND resi_project.locality_id in ($LocIdList) 
                AND  resi_project.project_status_id in (".LAUNCHED_ID_7.", ".UNDER_CONSTRUCTION_ID_1.") 
                AND resi_project.launch_date >= 2012-10-01 AND b.size = 0" ), "select" => "count(distinct b.project_id) as PROJECT_COUNT"));
    $activeProjectWithUnderConstNLaunch = ResiProject::virtual_find('all',array('conditions'=>array("(status = 'Active' OR status = 'ActiveInCms') AND residential_flag = 'Residential' AND locality_id in ($LocIdList) AND  project_status_id in( ".UNDER_CONSTRUCTION_ID_1.",".LAUNCHED_ID_7." )" ),
        "select" => "count(*) as PROJECT_COUNT"));
   $percntProjectWithSize = (100-($projectWithLaunchGrtrOct2012->project_count/$activeProjectWithUnderConstNLaunch->project_count)*100);
   ...
   $percntProjWithGeo = (100-($latLongWithProject->project_count/$activeProject->project_count)*100);
   ...
   $query = "select count(*) as project_count from (SELECT a.project_id as PROJECT_COUNT FROM `resi_project` 
               ...
               WHERE ($active) 
                 ...
                 AND resi_project.d_availability != 0 group by a.project_id 
              having max(b.effective_date) < '$twoMonthBeforeDate') table_alias";
    $availableProjects = ResiProject::find_by_sql($query);
    
    $availableProjectpriceUpdated2monthOld = (100-($availableProjects->project_count/$activeProject->project_count)*100);
