...
if (isset($phaseId) && $phaseId != -1) {
    ...
    $option_ids = array();
    foreach ($phase_options_temp as $options)
        array_push($option_ids, $options->options_id);
    $bedrooms = ResiProjectOptions::optionwise_bedroom_details($option_ids, $phaseId);
    $bedrooms_hash = array();
    foreach ($bedrooms as $bed)
        $bedrooms_hash[$bed->unit_type] = explode(",", $bed->beds);
...
if ($_SERVER['REQUEST_METHOD']) {
    ...
    foreach ($phase_quantity as $quantity)
        $phase_quantity_hash[$quantity->unit_type] = $quantity->agg;
    ...
    if ($error_msg == '') {
        ...
        ResiProjectPhase::transaction(function() {
            ...
            if ($phaseId != '0') {
                ...
                if ($_POST['project_type_id'] == APARTMENTS || $_POST['project_type_id'] == VILLA_APARTMENTS || $_POST['project_type_id'] == PLOT_APARTMENTS || $_POST['project_type_id'] == COMMERCIAL || $_POST['project_type_id'] == SHOP || $_POST['project_type_id'] == OFFICE || $_POST['project_type_id'] == SHOP_OFFICE || $_POST['project_type_id'] == OTHER) {
