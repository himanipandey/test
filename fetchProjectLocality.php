    ...
    if(!empty($alloc))
    {
        foreach($alloc as $key => $val)
        {
            ...
            if($temp[0] == 'all')
            {
                ...
                /** Check whether the All Project Option of that city are already selected  
                 * If Yes then no Project will get Displayed  
                 * Else Find Project for that City 
                 */
                $chkSql = @mysql_query("SELECT project_id FROM rule_project_mappings AS rpm LEFT JOIN rule_locality_mappings AS rlm ON rpm.rule_id = rlm.rule_locality_mapping_id WHERE rlm.city_id = '$city_id' AND rlm.locality_id = '-1'");
